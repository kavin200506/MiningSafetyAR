...
# Filter Unity logcat output for Fire Extinguisher, GLTFast, Diagnostics, Taps, Raycasts, and Verdicts
adb logcat -s Unity | grep --line-buffered -i -E "extinguisher|gltfast|submesh|pink|shader|fire|TAP_DIAG|RAY_DIAG|SPAWN_DIAG|GRAB_VERDICT|DIAG|WARN|INFO" | while read -r line; do
    echo "$line" >> "$LOG_FILE"
