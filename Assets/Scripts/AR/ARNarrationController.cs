    ...
    public class ARNarrationController : MonoBehaviour
    {
        ...
        private static void AutoSpawn()
        {
            ...
            else
            {
                ...
                UnityEngine.Object.DontDestroyOnLoad(go);
                go.AddComponent<ARNarrationController>();
                Debug.Log("[INFO] ARNarrationController: No prefab found at Resources/Prefabs/ARNarrationController — auto-created runtime singleton instance and attached component directly. Loading audio clips via Resources fallback.");
            }
        }
        ...
        private void Start()
        {
            // NOTE: Do NOT trigger FirstPlaneFound narration here just because we're in an AR
            // scene — that played the "surface detected" line immediately on scene start,
            // before any plane was actually found. Narration should only fire from a real
            // signal: SubscribeToEvents() below checks for already-tracked planes, and
            // OnPlanesChanged handles the live detection event.
            SubscribeToEvents();
        }
        ...
        private void OnSceneLoaded(UnityEngine.SceneManagement.Scene scene, UnityEngine.SceneManagement.LoadSceneMode mode)
        {
            ...
            UnsubscribeFromEvents();
            // Same as Start(): no blind trigger here either — SubscribeToEvents() below only
            // narrates if a plane is already tracked, otherwise we wait for the real
            // OnPlanesChanged detection event.
            SubscribeToEvents();
        }
        ...
        private void TriggerCommand(NarrationEvent evt)
        {
            ...
            // Drop any stale queued repeats of the PREVIOUS event (e.g. MidpointNudge
            // re-enqueued every 10s while the player keeps walking) so this new milestone
            // is heard promptly instead of waiting behind a backlog of old repeats.
            if (narrationQueue.Count > 0)
            {
                Debug.Log($"[INFO] ARNarrationController Clearing {narrationQueue.Count} stale queued narration event(s) to prioritize '{evt}'.");
                narrationQueue.Clear();
            }
        ...
        private IEnumerator ProcessNarrationQueue()
        {
            ...
            while (narrationQueue.Count > 0)
            {
                ...
                // Wait for actual playback to finish rather than trusting AudioClip.length blindly
                // (mismatched/estimated clip durations would otherwise stall or truncate the queue).
                // clipLength is still used as a safety cap so a stuck AudioSource can't hang the queue forever.
                float clipLength = activeClip != null ? activeClip.length / 0.85f : 2.5f;
                float maxWait = clipLength + 1.0f;
                float waited = 0f;

                yield return null; // let Play() actually start before polling isPlaying

                while (waited < maxWait && IsNarrationAudioPlaying())
                {
                    waited += Time.deltaTime;
                    yield return null;
                }

                yield return new WaitForSeconds(0.3f); // brief gap before the next queued clip
            }

            isProcessingQueue = false;
        }

        private bool IsNarrationAudioPlaying()
        {
            if (LanguageManager.Instance != null) return LanguageManager.Instance.IsVoiceoverPlaying;
            return dedicatedAudioSource != null && dedicatedAudioSource.isPlaying;
        }
    }
}
