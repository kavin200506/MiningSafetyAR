...
namespace MiningSafetyAR.UI.Pages
{
    public class CertificatePageController : PageController
    {
        ...
        void Refresh()
        {
            ...
            string nameStr = worker != null ? worker.name : "Ramesh Kumar";
            string titleStr = mod != null ? (mod.title ?? moduleId) : (moduleId == "gas_leak" ? "Gas Leak & Confined Space" : "Fire & Explosion Safety");
            string orgStr = worker != null ? worker.organization : "Bharat Coking Coal Ltd";
            int scoreVal = prog != null ? prog.bestScore : (mod != null ? mod.bestScore : 88);
            bool passed = prog == null || prog.status == ModuleStatus.Completed || scoreVal >= 70;
        ...
        void OnDownloadCertificate()
        {
            try
            {
                ...
                // Generate QR Texture & encode to PNG
                string qrContent = BuildVerificationUrl(certIdStr, existing);
                ...
                Debug.Log($"[CertificatePage] ✅ Successfully exported Certificate PNG to local gallery path: '{savePath}' ({pngBytes.Length} bytes)");
            }
